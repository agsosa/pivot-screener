export default class DataManager {
  eventEmitter; // Used to subscribe other modules to data events (i.e. data updated event)
  ...
  addTicker(ticker) {
    if (
      !this._tickersList.find(
        (q) => q.symbol === ticker.symbol && q.market === ticker.market && q.exchange === ticker.exchange
      )
    )
      this._tickersList.push(ticker);
  }

  updateCandlesticks(ticker, timeframe, candlesticks) {
    ticker.candlesticks[timeframe + 'Candles'] = candlesticks;
  }

  emitUpdateEvent(market) {
    this.eventEmitter.emit('data_updated', market); // TODO: Add exchange parameter
  }

  getTickersListByMarketExchange(market, exchange) {
  ...
  getSymbolsList(markets) {
    ...
    this._tickersList.map((q) => {
      if (!markets) {
        ...
        try {
          let split = markets.replace(/\s/g, '').split(',');
          const bMarkets = split != null && Array.isArray(split);
          if (bMarkets) {
  ...
  /*
		...
		Return: ITickers[] or []
	*/
  getFilteredTickers(timeframes, markets, symbols) {
    try {
      let arrSymbols  = [];
      let arrMarkets = [];
      let arrTimeframes = [];
      ...
      let filtered = JSON.parse(JSON.stringify(this._tickersList)); // No tickersList mutation
