...
function getSymbolsList(markets = undefined) {
	...
	data.tickersList.map((q) => {
		if (!markets) {
			...
			if (bMarkets) {
				markets = markets.map((q) => q.toLowerCase());
...
function getFilteredTickers(timeframes, markets, symbols) {
	try {
		...
		if (bSymbols) symbols = symbols.map((q) => q.toLowerCase());
		if (bMarkets) markets = markets.map((q) => q.toLowerCase());
		if (bTimeframes) timeframes = timeframes.map((q) => q.toLowerCase());
