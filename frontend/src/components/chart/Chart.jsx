...
const Chart = () => {
    ...
    if (data) {
      if (data.candlesticks && data.candlesticks.hourlyCandles && data.candlesticks.dailyCandles) {
        ...
        if (!lastData.current) initializeDrawings(data);
          ...
          if (
            a[a.length - 1].timestamp !== b[b.length - 1].timestamp ||
            lastData.current.symbol !== data.symbol ||
            forceDrawingsRefresh
          ) {
  ...
  function initializeChart() {
    ...
    series.current.applyOptions({
      ...
      priceFormat: { type: 'price', minMove: 0.0000001, precision: 8 },
    });
