...
const Ticker = types
  ...
  .views((self) => ({
    ...
    getTimeframeCandlesticks(timeframe = 'any') {
      if (
        !self.candlesticks ||
        (timeframe !== 'any' && !Object.keys(self.candlesticks).includes(`${timeframe}Candles`))
      )
        return undefined;
      const candles =
        timeframe === 'any'
          ? self.candlesticks[Object.keys(self.candlesticks)[0]]
          : self.candlesticks[`${timeframe}Candles`];
      return candles;
    },
    ...
    getCPR(timeframe, future = false) {
      ...
      result.isTested = future
        ? false
        : inRange(result.bc, currSession.low, currSession.high) ||
          inRange(result.tc, currSession.low, currSession.high);
      ...
      else if (result.price_position === 'above')
        result.closestApproximation = percentDifference(currSession.low, result.tc);
